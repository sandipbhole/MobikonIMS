...
namespace Mobikon.IMS.Controllers
{
    public class DeviceController : Controller
    {
        ...
        public ActionResult Device(string sortOrder,string deviceTypeFilter, string productSerialFilter,string deviceDetailsFilter, string statusDeviceFilter, string deviceTypeSearch,string productSerialSearch,  string deviceDetailsSearch, int? page, string statusDeviceSearch, string paging)
        {
            ...
            //ViewBag.productSerialSearch = deviceDAL.GetProductSerialList(statusDevice:mC.CommonMobikonIMS.StatusDevice.NotSoldDevices.ToString(),pageName:"Device",showBlockedDevice:true);
            // ViewBag.statusDeviceSearch = statusDAL.GetStatusNameList(pageName: "Device");
            // ViewBag.deviceTypeSearch = deviceDAL.GetDeviceTypeList(pageName: "DashBoard");
        ...
        public ActionResult CreateDeviceTransaction(MIM.DeviceTransaction requestSetDeviceTransaction, string command)
        {
            ...
            if (command == "Save")
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if ((Session["Role"].ToString() != "Sales"|| Session["Role"].ToString() != "Accounts" || Session["Role"].ToString() != "Administrator") && requestSetDeviceTransaction.status == "Blocked")
                    {
                        ViewBag.Message = "Not authorized to create blocked status device transaction.";
                        return View(requestSetDeviceTransaction);
                    }

                    if (requestSetDeviceTransaction.status != "Blocked")
                    {
                        if (true == deviceDAL.CheckBlockDeviceByUser(requestSetDeviceTransaction.productSerial, requestSetDeviceTransaction.userName))
                        {
        ...
        public ActionResult UpdateDevice(MIM.Device requestSetDevice,string command)
        {
            ...
            if (command == "Save")
            {
                ...
                {
                    requestSetDevice.userName = Session["UserName"].ToString();
                    if ((Session["Role"].ToString() != "Sales"|| Session["Role"].ToString() != "Accounts") && requestSetDevice.status == "Blocked")
                    {                       
                        ViewBag.Message = "Not authorized to blocked this device.";
                        return View(requestSetDevice);
                    }

                    if (requestSetDevice.status != "Blocked")
                    {
                        if (true == deviceDAL.CheckBlockDeviceByUser(requestSetDevice.productSerial, requestSetDevice.userName))
                        {
                            ViewBag.Message = "You cannot change the status of this device. This device already blocked by another user.";
                            return View(requestSetDevice);
                        }
                    }
                    bool result = deviceDAL.EditDevice(requestSetDevice);
        ...
        public ActionResult EditDevice(MIM.Device requestSetDevice, string command)
        {
            ...
            if (command == "Save")
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    updateDevice = deviceDAL.EditDevice(requestSetDevice);
        ...
        public ActionResult EditDeviceTransaction(MIM.DeviceTransaction requestSetDeviceTransaction, string command)
        {
            ...
            if (command == "Save")
            {
                ...
                {
                    requestSetDeviceTransaction.userName = Session["UserName"].ToString();
                    if ((Session["Role"].ToString() == "Sales" || Session["Role"].ToString() == "Accounts" ) && requestSetDeviceTransaction.status == "Blocked")
                    {
                        ViewBag.Message = "Not authorized to blocked this device transaction.";
                        return View(requestSetDeviceTransaction);
                    }
                    if (requestSetDeviceTransaction.status != "Blocked")
                    {
                    ...
                    requestSetDeviceTransaction.dcDate = requestSetDeviceTransaction.deliveryDate;
                    requestSetDeviceTransaction.createdDate = System.DateTime.Now;
